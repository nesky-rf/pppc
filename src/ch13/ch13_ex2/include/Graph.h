...
struct Circle : Shape {
	...
	void draw_lines() const;
	Point n() const { return {point(0).x+r, point(0).y}; }
	Point s() const { return {point(0).x+r, point(0).y+2*r}; }
	Point e() const { return {point(0).x+2*r, point(0).y+r}; }
	Point w() const { return {point(0).x, point(0).y+r}; }
	Point center() const { return { point(0).x + r, point(0).y + r }; }
	Point ne() const { return {point(0).x+2*r, point(0).y};}
	Point se() const { return {point(0).x+2*r, point(0).y+2*r}; }
	Point sw() const { return {point(0).x, point(0).y+2*r}; }
...
struct Ellipse : Shape {
	...
	void draw_lines() const;
	Point n() const { return {point(0).x+h/2, point(0).y}; }
	Point s() const { return {point(0).x+h/2, point(0).y+w}; }
	Point e() const { return {point(0).x+h, point(0).y+w/2}; }
	Point west() const { return {point(0).x, point(0).y+w/2}; }
	...
	Point ne() const { return {point(0).x+h, point(0).y};}
	Point se() const { return {point(0).x+h, point(0).y+w}; }
	Point sw() const { return {point(0).x, point(0).y+w}; }
	Point nw() const { return {point(0).x, point(0).y}; }
...
struct Arc : Shape {
	Arc(Point p, int ww, int hh, int ii, int ee)
	: w {ww}, h {hh}, init{ii}, end{ee} {
		add(Point{p.x-ww, p.y-hh});
	}
	void draw_lines() const;
private:
...
struct Boxs : Box {
	Boxs(Point p, int ww, int hh, string s)
	: Box(p,ww,hh) {
		lab = s;
	}
	void draw_lines() const;
