...
public:
	...
	void del() { points.clear(); }		// deletes points for reuse
protected:
...
struct Function_s : Function {
	Function_s(Fct f, double r1, double r2, Point orig, int count = 100, double xscale = 25, double yscale = 25)
	: Function(f, r1, r2, orig, count, xscale, yscale), 
		f(f), r1(r1), r2(r2), origin(orig), count(count), xscale(xscale), yscale(yscale) {  }
	
	void reset();
	void set_func(Fct fn) { f= fn; reset(); }
	void set_range(double r1n, double r2n) {r1= r1n, r2= r2n; reset(); }
	void set_origin(Point pn) { origin= pn; reset(); }
	void set_count(int cn) { count= cn; reset(); }
	void set_scale(double xsn, double ysn) { xscale= xsn; yscale= ysn; reset(); }
private :
	Fct* f;
	double r1;
	double r2;
...
struct Inmobile_Circle : Circle {
	Inmobile_Circle(Point p, int rr)
	: Circle(p,rr) {}

	void move(int dx, int dy) { error("Inmobile_Circle: Operation not permitted"); }
};
...
struct Smileyh : Smiley {
	Smileyh(Point p, int rr)
	: Smiley(p,rr) {}
